...
if ($del == "del" && ($admintype == 'AD' || $admintype == 'DV')) {
  ...
  $stmt = $pdo->prepare("DELETE FROM stafflist WHERE slil = ? LIMIT 1");
  $stmt->execute([$which]); 
  if ($stmt->rowCount() > 0) {
...
if ($newadmin == 'newadmin') {
  ...
  $insert_stmt = $pdo->prepare("INSERT INTO testimonials (test_txt, salutation, sender, company_name, date_created, image, sort_order) VALUES (?, ?, ?, ?, ?, ?, ?)");
  $insert_stmt->execute([$test_txt, $salutation, $sender, $company_name, $today, $valueblank, $sort_order]);
  ...
  if (is_uploaded_file($_FILES['Image1']['tmp_name'])) {
     ...
     $large_image_path = '../testimonial/'.$generateID.'.jpg';
        $actualname = $generateID.'.jpg';

     $result = resize_save_jpeg( $temp_image_path, $large_image_path, 200, 200 );
     if ( $result )
     {
      $stmt = $pdo->prepare("UPDATE testimonials SET image = ? WHERE did = ?"); 
               ...
               <div class="col-xl-12">
                  <div class="table-responsive">
                      <table class="table table-striped my-4 w-100" id="maintable">
                         <thead>
                            <tr>
                                <th class="sort-alpha" data-priority="1">Sender</th>
                                <th>Company</th>
                         ...
                         <tbody>
<?PHP
$stmt = $pdo->prepare("SELECT did, sender, company_name, date_created, image, sort_order FROM testimonials");
  $stmt->execute();
...
?>
<tr>
  <td valign="top"><a href="testimonialdetail.php?which=<?php echo $did ?>&amp;change=no"><?php echo $sender ?></a></td>
  <td valign="top"><?php echo $company_name ?></td>
  <td valign="top"><?php echo $sort_order ?></td>
  <td valign="top"><?php echo date('j M Y',$date_created) ?></td>
                     ...
                     <div class="card border-info">
                        ...
                        <div class="card-body">
                          <div class="form-group">
                              ...
                              <input class="form-control" type="text" id="sender" name="sender" required>
                           </div>
                           <div class="form-group">
                              <label class="col-form-label" for="salutation">Position</label>
                              <input class="form-control" type="text" id="salutation" name="salutation">
                           </div>
                           <div class="form-group">
                           ...
                           <div class="form-group">
                              ...
                              <textarea class="form-control summernote" type="text" id="test_txt" name="test_txt"></textarea>
                           </div>
                           <div class="form-group">

...
if ($del == "del" && ($admintype == 'AD' || $admintype == 'DV')) {
  ...
  $stmt = $mysqli->prepare("SELECT company_name, logo FROM portfolio WHERE pid = ?");
  $stmt->bind_param("i", $which);
  $stmt->execute();
  $stmt->store_result();
  $stmt->bind_result($company_name_del, $filename_id_del);
  $stmt->fetch();
  $stmt->close();

  // delete 
  $stmt = $mysqli->prepare("DELETE FROM portfolio WHERE pid = ? LIMIT 1");
  $stmt->bind_param("i", $which); 
  $stmt->execute();
...
if ($newadmin == 'newadmin') {
  ...
  // write new record
  $insert_stmt = $mysqli->prepare("INSERT INTO portfolio (sort_order, logo, company_name) VALUES (?, ?, ?)");
  $insert_stmt->bind_param("iss", $sort_order, $valueblank, $company_name);
  $insert_stmt->execute();
               ...
               <div class="col-xl-12">
                  <div class="table-responsive">
                        <table class="table table-striped my-4 w-100" id="maintable">
                           ...
                           <tbody>
<?PHP
$tableset = $mysqli->prepare("SELECT pid, company_name, sort_order FROM portfolio ");
$tableset->execute();
$tableset->store_result();
$tableset->bind_result($pid, $company_name, $tablesort_order);
